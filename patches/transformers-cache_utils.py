...
class DynamicLayer(CacheLayerMixin):
    ...
    def lazy_initialization(self, key_states: torch.Tensor):
        ...
        # PATCH: пустой тензор с shape совпадающим с key_states кроме seq dim (dim=-2)
        if key_states.dim() == 4:
            ...
            empty_shape = (0, key_states.shape[1])
        else:
            empty_shape = (0,)
        self.keys = torch.empty(empty_shape, dtype=self.dtype, device=self.device)
        self.values = torch.empty(empty_shape, dtype=self.dtype, device=self.device)
